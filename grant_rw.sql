...
SELECT DISTINCT 'GRANT SELECT,INSERT,UPDATE,DELETE,TRUNCATE ON TABLE ' || schemaname || '.' || '"' || tablename || '"' || ' TO ' || quote_ident(r.param_role_name) || ';' As sql
...
UNION ALL
SELECT DISTINCT 'GRANT ALL ON FUNCTION ' || routine_schema || '.' || '"' || routine_name || '"' || '('
...
UNION ALL
SELECT 'GRANT ALL ON SEQUENCE ' || sequence_schema || '.' || '"' || sequence_name || '"' || ' TO ' || quote_ident(r.param_role_name) || ';' As sql
FROM information_schema.sequences CROSS JOIN r
UNION ALL
SELECT 'GRANT ALL ON SCHEMA ' || '"' || schema_name || '"' || ' TO ' || quote_ident(r.param_role_name) || ';' As sql
